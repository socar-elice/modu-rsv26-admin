  ...
  var PAGE_INFO = {
    '300_주차상품판매정책설정.html': {
      ...
      changes: [
        ...
        '구매가능 시작/마감: 시간/일 토글(segment) 제거 → "N일 전 HH:MM시 까지" 형식으로 단순화',
        '고객 추가 정보 입력 항목 섹션 추가',
        '주차권유형 시간권(입차기준)추가',
        '기본 정보 섹션 MVP 배지 표시 (파란색 하이라이트)',
        '기본 판매정책 / 공휴일 판매정책 섹션 3차 배지 표시 (보라색 하이라이트)',
        '네비메뉴 배지: 제휴상품생성하기·제휴상품상세 → MVP, 변경 배지 회색 스타일 차등화',
      ],
      userStories: [
    ...
    '320_제휴상품상세.html': {
      ...
      changes: [
        ...
        '판매정책(기본+공휴일) 기간별 통합 등록 기능 추가 (기간 겹침 방지, 종료일 선택)',
        '신규 판매정책 등록 시 종료일 없는 기존 정책의 종료일을 자동으로 신규 시작일 전날로 설정',
        '기간별 판매정책 수정 기능 추가 (수정 버튼 → 저장/취소)',
        '요일별/일괄 모드 전환은 수정 모드에서만 가능',
        '공휴일 판매정책을 요일별/일괄 관계없이 독립 섹션으로 분리',
        '판매정책에서 이용가능 시작/종료시간 항목 제거',
        '기간별 판매정책에서 상태 컬럼 제거 (등록 즉시 적용)',
        '판매정책 좌우 분할 UI (왼쪽: 기간 리스트, 오른쪽: 선택 기간 설정)',
        '구매가능 시작/마감: 시간/일 토글(segment) 제거 → "N일 전 HH:MM시 까지" 형식으로 단순화',
      ],
      policies: [
        '내용을 곧 업로드할 예정입니다.',
    ...
    '325_판매정책_캘린더뷰.html': {
      title: '판매정책 캘린더뷰',
      description: '제휴상품의 이벤트 스케줄과 기본 판매정책을 캘린더 형태로 시각화하는 화면입니다. 320 제휴상품상세에서 연결됩니다.',
      changes: [
        '월간 캘린더에 날짜별 판매가격/정산금액/수량 표시',
        '이벤트 스케줄 vs 기본정책 vs 공휴일정책 시각적 구분 (배지 색상)',
        '날짜 클릭 시 적용 우선순위 상세 패널 (이벤트 > 공휴일 > 기본정책)',
        '기간별 정책 모드(요일별/일괄) 자동 반영',
        '판매중지(수량 0) 및 미설정 기간 표시',
      ],
      policies: [
        '내용을 곧 업로드할 예정입니다.',
      ],
